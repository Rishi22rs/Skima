    ...
    render() {
        ...
        <React.Fragment>
            <ul id="slide-out" className="sidenav sidenav-fixed" style={palette.background}>
                ...
                <li onClick={()=>window.closeSidenav()} className='hide-on-med-and-down'><Link to={`/HeyWasup/timetable/${localStorage.getItem('cookie')}`} className='waves-effect' style={palette.heading}><i className="material-icons" style={palette.heading}>schedule</i>Timetable</Link></li>
                <li onClick={()=>window.closeSidenav()} className='hide-on-med-and-down'><Link to={`/HeyWasup/planner/${localStorage.getItem('cookie')}`} className='waves-effect' style={palette.heading}><i className="material-icons" style={palette.heading}>event</i>Planner</Link></li>
